...
entering its i32 arguments followed by its name, such as `20 22 add`. Definitions
...
and `13` final signature mismatch.

The bootstrap currently limits definitions/table slots to 256, the semantic
type stack to 1024 entries, and source to 1 MiB. Named non-function dictionary
objects and indirect dictionary compiler-action dispatch remain staged work in
`TODO.md`; dictionary entries are not architecturally restricted to runtime
functions.
